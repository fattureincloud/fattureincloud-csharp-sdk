...
namespace It.FattureInCloud.Sdk.Test.Api
{
    ...
    {
        private readonly string createIssuedDocumentResponseBody;
        private readonly string getEmailDataResponseBody;
        private readonly string getExistingIssuedDocumentTotalsResponseBody;
        private readonly string getIssuedDocumentPreCreateInfoResponseBody;
        private readonly string getIssuedDocumentResponseBody;
        private readonly string getNewIssuedDocumentTotalsResponseBody;
        private readonly string listIssuedDocumentsResponseBody;
        private readonly string modifyIssuedDocumentResponseBody;
        private readonly string uploadIssuedDocumentAttachmentResponseBody;
        private Mock<IIssuedDocumentsApi> instance = new Mock<IIssuedDocumentsApi>();
        ...
        public IssuedDocumentsApiTests()
        {
            ...
            var listArchiveCategoriesResponse =
                JsonConvert.DeserializeObject<CreateIssuedDocumentResponse>(createIssuedDocumentResponseBody);
            instance
            ...
            getNewIssuedDocumentTotalsResponseBody =
                "{ 'data': { 'amount_net': 68.18, 'amount_rivalsa': 13.64, 'amount_net_with_rivalsa': 81.82, 'taxable_amount': 81.82, 'amount_vat': 8.18, 'amount_gross': 90.0, 'amount_due': 90.0, 'is_enasarco_maximal_exceeded': false, 'payments_sum': 75.0, 'vat_list': {} }}";
            var getNewIssuedDocumentTotalsResponse =
                JsonConvert.DeserializeObject<GetNewIssuedDocumentTotalsResponse>(
                    getNewIssuedDocumentTotalsResponseBody);
            instance
                .Setup(p => p.GetNewIssuedDocumentTotals(Moq.It.IsAny<int>(),
                    Moq.It.IsAny<GetNewIssuedDocumentTotalsRequest>()))
                .Returns(getNewIssuedDocumentTotalsResponse);

            listIssuedDocumentsResponseBody =
                "{ 'current_page': 1, 'first_page_url': 'page=1', 'from': 1, 'last_page': 1, 'last_page_url': 'page=1', 'path': 'issued_documents', 'per_page': 50, 'to': 2, 'total': 2, 'data': [ { 'type': 'receipt', 'id': 12345, 'number': 1, 'numeration': 'rec123', 'date': '2021-08-20', 'subject': '', 'visible_subject': '', 'use_split_payment': false, 'use_gross_prices': false, 'e_invoice': false, 'show_payments': false, 'show_payment_method': false, 'show_paypal_button': false, 'show_notification_button': false, 'delivery_note': false, 'accompanying_invoice': false, 'is_marked': false, 'next_due_date': '2021-12-31', 'url': 'ypbqqe4u8w8bdabcd5fd5b1a4gtqhlof.pdf' }, { 'type': 'receipt', 'id': 12346, 'number': 1, 'numeration': 'rec124', 'date': '2021-08-28', 'subject': '', 'visible_subject': '', 'use_split_payment': false, 'use_gross_prices': false, 'e_invoice': false, 'show_payments': false, 'show_payment_method': false, 'show_paypal_button': false, 'show_notification_button': false, 'delivery_note': false, 'accompanying_invoice': false, 'is_marked': false, 'next_due_date': '2021-12-31', 'url': 'qwertye4u8w8bdaqrt5fd5b1a4gtqhlof.pdf' } ]}";
            var listIssuedDocumentsResponse =
                JsonConvert.DeserializeObject<ListIssuedDocumentsResponse>(listIssuedDocumentsResponseBody);
            instance
                .Setup(p => p.ListIssuedDocuments(Moq.It.IsAny<int>(), Moq.It.IsAny<string>(), Moq.It.IsAny<string>(),
                    Moq.It.IsAny<string>(), Moq.It.IsAny<string>(), Moq.It.IsAny<int?>(), Moq.It.IsAny<int?>(),
                    Moq.It.IsAny<string>()))
                .Returns(listIssuedDocumentsResponse);

            modifyIssuedDocumentResponseBody =
                "{ 'data': { 'type': 'receipt', 'show_totals': 'all', 'id': 12345, 'number': 1, 'numeration': 'rec123', 'date': '2021-08-20', 'year': 2021, 'currency': { 'id': 'EUR', 'symbol': '€', 'exchange_rate': '1.00000' }, 'language': { 'code': 'it', 'name': 'Italiano' }, 'subject': '', 'visible_subject': '', 'rc_center': '', 'notes': '', 'withholding_tax_taxable': 100.0, 'payment_method': { 'id': 4, 'name': 'Credit card', 'is_default': false, }, 'use_split_payment': false, 'use_gross_prices': false, 'e_invoice': false, 'items_list': [ { 'product_id': 5432, 'code': 'SG3', 'name': 'Soggiorno', 'description': '', 'qty': 1.0, 'measure': '', 'net_price': 68.18182, 'gross_price': 75.0, 'vat': { 'id': 3, 'value': 10.0, 'description': '', 'e_invoice': false, 'is_disabled': false }, 'not_taxable': false, 'apply_withholding_taxes': true, 'discount_highlight': false, 'in_ddt': false, 'stock': false } ], 'payments_list': [ { 'status': 'not_paid', 'due_date': '2020-08-23', 'amount': 75.0, 'paid_date': '2020-08-23' } ], 'template': { 'id': 2821, 'name': 'Light Smoke' }, 'h_margins': 15, 'v_margins': 16, 'show_payments': true, 'show_payment_method': false, 'show_paypal_button': true, 'show_notification_button': false, 'show_tspay_button': false, 'delivery_note': false, 'accompanying_invoice': false, 'is_marked': false, 'next_due_date': '2020-08-23', 'url': 'y12h45rn9yf2mse0p43t7ec90vr.pdf' } }";
            var modifyIssuedDocumentResponse =
                JsonConvert.DeserializeObject<ModifyIssuedDocumentResponse>(modifyIssuedDocumentResponseBody);
            instance
                .Setup(p => p.ModifyIssuedDocument(Moq.It.IsAny<int>(), Moq.It.IsAny<int>(),
                    Moq.It.IsAny<ModifyIssuedDocumentRequest>()))
                .Returns(modifyIssuedDocumentResponse);

            uploadIssuedDocumentAttachmentResponseBody =
                "{'data':{'attachment_token':'YmMyNWYxYzIwMTU3N2Y4ZGE3ZjZiMzg5OWY0ODNkZDQveXl5LmRvYw'}}";
            var uploadIssuedDocumentAttachmentResponse =
                JsonConvert.DeserializeObject<UploadIssuedDocumentAttachmentResponse>(
                    uploadIssuedDocumentAttachmentResponseBody);
            instance
                .Setup(p => p.UploadIssuedDocumentAttachment(Moq.It.IsAny<int>(), Moq.It.IsAny<string>(),
                    Moq.It.IsAny<Stream>()))
        ...
        public void GetExistingIssuedDocumentTotalsTest()
        {
            ...
            var getExistingIssuedDocumentTotalsRequest = new GetExistingIssuedDocumentTotalsRequest();

            var response =
                instance.Object.GetExistingIssuedDocumentTotals(companyId, documentId,
        ...
        {
            var companyId = 2;
            var getNewIssuedDocumentTotalsRequest = new GetNewIssuedDocumentTotalsRequest();
        ...
        public void ModifyIssuedDocumentTest()
        {
            var companyId = 2;
            var documentId = 12345;
            var modifyIssuedDocumentRequest = new ModifyIssuedDocumentRequest();
